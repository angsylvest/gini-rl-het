...
class Runner(object):
    ...
    def __init__(self, config:Dict):
        ...
        if self.algorithm_name in ["rmappo"]:  
        
            from onpolicy.algorithms.graph_mappo import GR_MAPPO as TrainAlgo
            from onpolicy.algorithms.graph_MAPPOPolicy import GR_MAPPOPolicy as Policy_gnn
        if self.algorithm_name in ["mat"]:
            print("name of the algorithm is: ", self.algorithm_name)
            from onpolicy.algorithms.graphTransfomer_Policy import GR_MAPPOPolicy as Policy_transfomer
            from onpolicy.algorithms.mat_trainer import MATTrainer as TrainAlgo_Mat    
...
#************************** Initialize Policy  ********************************************#
        ...
        # policy network rmappo
        if self.algorithm_name in ["rmappo"]:  
            self.policy = Policy_gnn(self.all_args,
        ...
        # polciy network mat    
        if self.algorithm_name in ["mat"]:
            self.policy = Policy_transfomer(self.all_args,
                    self.envs.observation_space[0],
                    share_observation_space,
                    self.envs.node_observation_space[0],
                    self.envs.edge_observation_space[0],
...
#************************** Initialize Trainer and Buffer **********************************#
        ...
        if self.algorithm_name in ["mat"]:
            # algorithm
            self.trainer = TrainAlgo_Mat(self.all_args, self.policy, self.num_agents, device=self.device)            
