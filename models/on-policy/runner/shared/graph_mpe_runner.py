...
class GMPERunner(Runner):
    ...
    def collect(self, step:int) -> Tuple[arr, arr, arr, arr, arr, arr]:
        ...
        # type of self.trainer <class 'onpolicy.algorithms.mat_trainer.MATTrainer'>
        print(" type of self.trainer", type(self.trainer))
        
        #  type of self.trainer.policy <class 'onpolicy.algorithms.graphTransfomer_Policy.GR_MAPPOPolicy'>
        print(" type of self.trainer.policy", type(self.trainer.policy))
        ...
        print(" rnn_ state shape", rnn_states.shape)
        print(" from graph mpe runner collect function: action shape", action.shape)    
        # [self.envs, agents, dim]
        
        # this is where the our problem is coming from. This is why we are getting (128, 9, 1) instead of (128, 3, 1)
        values = np.array(np.split(_t2n(value), self.n_rollout_threads))
        actions = np.array(np.split(_t2n(action), self.n_rollout_threads))
        # actions = action.reshape(384, 3, 1)
        # values = value.reshape(384, 3, 1)
        
        # actions = actions.cpu().numpy()
        # values = values.cpu().numpy()


        print(" action shape after split", actions.shape)
    ...
    def insert(self, data):
        ...
        print('Shape of rnn_states ahhhhhhhhhhhhh:', rnn_states.shape)
        print('Shape of dones ahhhhhhhhhhhhh:', dones.shape)
        print('Number of True in dones ahhhhhhhhhhhhh: ', (dones == True).sum())
        print('Shape of zeros array ahhhhhhhhhhhhh:', np.zeros(((dones == True).sum(), self.recurrent_N, self.hidden_size), dtype=np.float32).shape)
        print('Shape of selected rnn_states ahhhhhhhhhhhhh:', rnn_states[dones == True].shape)
